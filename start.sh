#!/bin/bash
#
# Start script for ocr-api-consumer
...
if [[ -z "${MESOS_SLAVE_PID}" ]]; then
    ...
    source ~/.chs_env/ocr-api-consumer/env

    PORT="${OCR_API_CONSUMER_PORT}"
else
    PORT="$1"
    CONFIG_URL="$2"
    ENVIRONMENT="$3"
...
exec java ${JAVA_MEM_ARGS} -jar -Dserver.port="${PORT}" "${APP_DIR}/ocr-api-consumer.jar"
